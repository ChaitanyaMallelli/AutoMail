...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        ...
        try { db.Database.ExecuteSqlRaw("UPDATE \"Resumes\" SET \"FilePath\" = 'Resume/Sai_Prasad_SRE_Resume.pdf' WHERE \"IsActive\" = true AND (\"FilePath\" IS NULL OR \"FilePath\" = '');"); } catch { }
        ...
        try
        {
            if (!db.UserProfiles.Any())
            {
                ...
                {
                    Id = 1,
                    FullName = "Sai Prasad Dasari",
                    Email = "saidasari357@gmail.com",
                    Phone = "+91 9326046360",
                    LinkedInUrl = "https://www.linkedin.com/in/saiprasaddasari/"
                });
                db.SaveChanges();
            }
        }
